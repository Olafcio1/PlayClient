package pl.olafcio.playclient.theme;
...
import pl.olafcio.playclient.theme.screens.PlayModulesScreen;
import pl.olafcio.playclient.theme.widgets.*;
import pl.olafcio.playclient.theme.widgets.input.WPlayDropdown;
import pl.olafcio.playclient.theme.widgets.input.WPlaySlider;
import pl.olafcio.playclient.theme.widgets.input.WPlayTextBox;
import pl.olafcio.playclient.theme.widgets.pressable.*;

import static meteordevelopment.meteorclient.MeteorClient.mc;

public class PlayGuiTheme extends GuiTheme {
    ...
    public final Setting<SettingColor> favoriteColor = color("favorite", "Color of checked favorite button.", new SettingColor(161, 82, 212));
    ...
    // Widgets
    @Override
    public WPlayWindow window(WWidget icon, String title) {
        return w(new WPlayWindow(icon, title));
    }

    @Override
    public WLabel label(String text, boolean title, double maxWidth) {
        if (maxWidth == 0 && !text.contains("\n")) return w(new WPlayLabel(text, title));
        return w(new WPlayMultiLabel(text, title, maxWidth));
    }

    @Override
    public WHorizontalSeparator horizontalSeparator(String text) {
        return w(new WPlayHorizontalSeparator(text));
    }

    @Override
    public WVerticalSeparator verticalSeparator() {
        return w(new WPlayVerticalSeparator());
    }

    @Override
    protected WButton button(String text, GuiTexture texture) {
        return w(new WPlayButton(text, texture));
    }

    @Override
    protected WConfirmedButton confirmedButton(String text, String confirmText, GuiTexture texture) {
        return w(new WPlayConfirmedButton(text, confirmText, texture));
    }

    @Override
    public WMinus minus() {
        return w(new WPlayMinus());
    }

    @Override
    public WConfirmedMinus confirmedMinus() {
        return w(new WPlayConfirmedMinus());
    }

    @Override
    public WPlus plus() {
        return w(new WPlayPlus());
    }

    @Override
    public WCheckbox checkbox(boolean checked) {
        return w(new WPlayCheckbox(checked));
    }

    @Override
    public WSlider slider(double value, double min, double max) {
        return w(new WPlaySlider(value, min, max));
    }

    @Override
    public WTextBox textBox(String text, String placeholder, CharFilter filter, Class<? extends WTextBox.Renderer> renderer) {
        return w(new WPlayTextBox(text, placeholder, filter, renderer));
    }

    @Override
    public <T> WDropdown<T> dropdown(T[] values, T value) {
        return w(new WPlayDropdown<>(values, value));
    }

    @Override
    public WTriangle triangle() {
        return w(new WPlayTriangle());
    }

    @Override
    public WTooltip tooltip(String text) {
        return w(new WPlayTooltip(text));
    }

    @Override
    public WView view() {
        return w(new WPlayView());
    }

    @Override
    public WSection section(String title, boolean expanded, WWidget headerWidget) {
        return w(new WPlaySection(title, expanded, headerWidget));
    }

    @Override
    public WAccount account(WidgetScreen screen, Account<?> account) {
        return w(new WPlayAccount(screen, account));
    }

    @Override
    public WWidget module(Module module, String title) {
        return w(new WPlayModule(module, title));
    }

    @Override
    public WQuad quad(Color color) {
        return w(new WPlayQuad(color));
    }

    @Override
    public WTopBar topBar() {
        return w(new WPlayTopBar());
    }

    @Override
    public WFavorite favorite(boolean checked) {
        return w(new WPlayFavorite(checked));
    }
    ...
    public TabScreen modulesScreen() {
        return new PlayModulesScreen(this);
    }

    @Override
    public boolean isModulesScreen(Screen screen) {
        return screen instanceof PlayModulesScreen;
    }
